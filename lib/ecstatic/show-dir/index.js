  ...
  return function middleware(req, res, next) {
    ...
    fs.stat(dir, (statErr, stat) => {
      ...
      fs.readdir(dir, (readErr, _files) => {
        ...
        function isIgnoredFile(ignores, regexs, file) {
          var result = false;
        ...
        function isAllowedDir(allows, pathname, dir) {
          if (allows.length === 0) {
            return true;
          }
          if (dir === '..') {
          ...
          allows.forEach(function (allow) {
            allow = path.join(opts.root, allow);
            let item = path.join(opts.root, pathname, dir);
            if (item.indexOf(allow) === 0) {
              result = true;
            }
          });
          return result;
        ...
        function render(dirs, renderFiles, lolwuts) {
          ...
          const failed = false;
          const writeRow = (file) => {
            // render a row given a [name, stat, filepath] tuple
            const isDir = file[1].isDirectory && file[1].isDirectory();

            if (!isIgnoredFile(opts.config[req.user || 'admin'].ignore, opts.config[req.user || 'admin'].ignoreRegex, file[0])
                && ( !isDir
                  || isAllowedDir(opts.config[req.user || 'admin'].allows, pathname, file[0])
                )
            ) {
              ...
              html += `${'<tr>' +
                ...
                `<td class="file-size"><code>${moment(file[1].mtime).fromNow()}</code></td>` +
                '</tr>\n';
          ...
          html += `<br><address>Node.js ${
            process.version
            }/ <a href="https://github.com/hufyhang/node-ecstatic">ecstatic-htpasswd</a> ` +
            `server running @ ${
